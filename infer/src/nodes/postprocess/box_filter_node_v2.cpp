...
void BoxFilter_v2::on_cv_image(const std::shared_ptr<msgs::cv_frame> &frame) {
    ...
    min_width_ *= 0 < min_width_ && min_width_ <= 1.0f ? frame->frame_info->width() : 1;
    max_width_ *= 0 < max_width_ && max_width_ <= 1.0f ? frame->frame_info->width() : 1;
    min_height_ *= 0 < min_height_ && min_height_ <= 1.0f ? frame->frame_info->height() : 1;
    max_height_ *= 0 < max_height_ && max_height_ <= 1.0f ? frame->frame_info->height() : 1;
    min_area_ *= 0 < min_area_ && min_area_ <= 1.0f ? frame->frame_info->width() * frame->frame_info->height() : 1;
    max_area_ *= 0 < max_area_ && max_area_ <= 1.0f ? frame->frame_info->width() * frame->frame_info->height() : 1;
    ...
    clone_frame->check_report_callback_ =
        [callback = clone_frame->check_report_callback_](const std::vector<FrameExtInfo> &ext_info) {
            ...
            if (ext_info.back().algo_type == AlgoType::kSegmentation) {
                status = ext_info.back().map_seg_info.empty() ? FrameType::kBase : FrameType::kReport;
            }
