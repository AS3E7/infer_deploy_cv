...
void Report_v2::on_setup() {
    ...
    draw_image_->init_drawing("/home/config/NotoSansCJK-Regular.ttc", "/home/resources/background.json");
}

void Report_v2::on_report(const std::shared_ptr<msgs::cv_frame> &frame) {
#if defined(WITH_RV1126)
    if (!jpeg_decoder_) {
        jpeg_decoder_ = std::make_unique<codec::RkMediaJpegCodec>();
    ...
    if (frame->task_type == TaskType::kCamera) {
        ...
        if (real_time_push_) {
            ...
            if (codec_type_ == "h264" || codec_type_ == "hevc") {
                ...
                async_result_ = std::async(std::launch::async, [this, frame, event_id]() {
                    std::this_thread::sleep_for(std::chrono::seconds(save_time_ / 2));
...
#if defined(WITH_RV1126)
                    jpeg_decoder_->save_image(frame->frame_info->src_frame->data->data,
                                              (event_folder_ + "/" + event_id + ".jpg").c_str());
#elif defined(WITH_TX5368)
                        jpeg_decoder_->save_image(frame->frame_info->src_frame->data,
                                          (event_folder_ + "/" + event_id + ".jpg").c_str());
#else
                        image_wrapper::image_save_as_jpeg(frame->frame_info->src_frame->data,
                                                  event_folder_ + "/" + event_id + ".jpg", image_quality_);
#endif
        ...
        if (frame->frame_type == FrameType::kNone) {
            ...
            auto event_id = boost::uuids::to_string(boost::uuids::random_generator()());
            auto buffer = frame_info_to_string(frame->task_name, event_id, time(NULL), prev_frame_info_);

            // 保存最后一帧识别结果
            auto url = report_url_.substr(0, report_url_.find_last_of('.'));
            std::ofstream ofs(url + ".json");
            ofs << buffer;
            ofs.close();
            ...
            if (!export_video_) {
                ...
                export_video_ = std::make_unique<ExportVideo>();
                if (!export_video_->init_video(report_url_, frame->infer_frame_rate, frame->frame_info->width(),
            ...
            prev_frame_info_ = frame->frame_info;
        }
    } else if (frame->task_type == TaskType::kImage) {
        draw_image_->draw_frame(frame->frame_info);
        create_directories(report_url_.substr(0, report_url_.find_last_of('/')));
#if defined(WITH_RV1126)
        jpeg_decoder_->save_image(frame->frame_info->tgt_frame.data, report_url_.c_str());
#else
...
std::string Report_v2::frame_info_to_string(const std::string &task_name, const std::string &event_id,
    ...
    if (last_ext_info.algo_type == AlgoType::kSegmentation) {
        ...
        for (const auto &[_, item] : last_ext_info.map_seg_info) {
            for (const auto &[label, contours] : item.seg_contours) {
        ...
        // auto seg_map = std::unique_ptr<bm_image, void (*)(bm_image *)>(new bm_image, [](bm_image *ptr) {
        //     bm_image_destroy(*ptr);
        //     delete ptr;
        // });
        // bmcv_rect_t rect{0, 0, frame_info->width(), frame_info->height()};
        // bm_image_create(*image_wrapper::bm_handle, frame_info->height(), frame_info->width(), FORMAT_GRAY,
        //                 DATA_TYPE_EXT_1N_BYTE, seg_map.get(),
        //                 image_wrapper::get_bm_stride(FORMAT_GRAY, frame_info->width()).data());
        // bm_image_alloc_dev_mem_heap_mask(*seg_map, 6);
        // auto ptr = last_ext_info.seg_map.data();
        // bm_image_copy_host_to_device(*seg_map, (void **)&ptr);

        // auto mask = std::unique_ptr<bm_image, void (*)(bm_image *)>(new bm_image, [](bm_image *ptr) {
        //     bm_image_destroy(*ptr);
        //     delete ptr;
        // });
        // bmcv_image_threshold(*image_wrapper::bm_handle, *seg_map, *mask, 0, 255, BM_THRESH_BINARY);

        // auto masked_img = std::unique_ptr<bm_image, void (*)(bm_image *)>(new bm_image, [](bm_image *ptr) {
        //     bm_image_destroy(*ptr);
        //     delete ptr;
        // });
        // bmcv_image_bitwise_and(*image_wrapper::bm_handle, *frame_info->src_frame->data, *mask, *masked_img);

        // auto result = std::shared_ptr<bm_image>(new bm_image, [](bm_image *ptr) {
        //     bm_image_destroy(*ptr);
        //     delete ptr;
        // });
        // bmcv_image_bitwise_or(*image_wrapper::bm_handle, *masked_img, *seg_map, *result);

        // image_wrapper::image_save_as_jpeg(result, event_folder_ + "/" + event_id + ".jpg", image_quality_);

        // cv::Mat mat_image;
        // if (cv::bmcv::toMAT(frame_info->src_frame->data.get(), mat_image) != BM_SUCCESS) { throw std::runtime_error("Failed to convert bm_image"); }

        // cv::Mat binary_mask;
        // auto seg_map = cv::Mat(frame_info->height(), frame_info->width(), CV_8UC1);
        // memcpy(seg_map.data, last_ext_info.seg_map.data(), last_ext_info.seg_map.size());
        // cv::threshold(seg_map, binary_mask, 0, 255, cv::THRESH_BINARY);

        // cv::Mat masked_img;
        // cv::bitwise_and(mat_image, mat_image, masked_img, binary_mask);
        // cv::Mat result;
        // cv::bitwise_or(masked_img, seg_map, result);
        // cv::imwrite(event_folder_ + "/" + event_id + ".jpg", result);

        // std::vector<std::vector<cv::Point>> contours;
        // cv::findContours(binary_mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
        // if (contours.size() == 0) { throw std::runtime_error("11111"); }
        // for (const auto &item : contours) {
        //     cv::Rect rect = cv::boundingRect(item);
        //     printf("%d, %d, %d, %d\n", rect.x, rect.y, rect.width, rect.height);
        //     cv::rectangle(mat_image, rect, cv::Scalar(0, 255, 0), 2);
        //     printf("===============================================================\n");
        // }
        // cv::imwrite(event_folder_ + "/" + event_id + ".jpg", mat_image);

#endif
    } else {

        const auto &front_ext_info = frame_info->ext_info.front();
        for (const auto &[_, target] : front_ext_info.map_target_box) {
            auto target_obj = nlohmann::json::object();
            target_obj["id"] = target.track_id;
            target_obj["prev_id"] = target.prev_id;

            target_obj["label"] = front_ext_info.map_class_label.at(target.class_id);
            ...
            target_obj["color"] = front_ext_info.map_class_color.at(target.class_id).to_array<int>();
