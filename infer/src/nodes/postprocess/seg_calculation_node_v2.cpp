...
void SegCalculation_v2::on_cv_image(const std::shared_ptr<msgs::cv_frame> &frame) {
    ...
    if (enable_area_ || enable_length_) {
        ...
        for (auto &[_, item] : back_ext_info.map_seg_info) {
            memcpy(input_mask.data, item.seg_map.data(), item.seg_map.size());
            bev_trans_->PostProcess(input_mask, &output_mask);
            seg_by_contours_->PostProcess(
                output_mask, vec_labels,
                const_cast<std::map<uint8_t, std::vector<nodes::SegContour>> &>(item.seg_contours));
        }
        ...
        for (auto &[_, item] : back_ext_info.map_seg_info) {
            ...
            cv::Mat input_mask = cv::Mat(item.seg_height, item.seg_width, CV_8UC1);
            memcpy(input_mask.data, item.seg_map.data(), item.seg_map.size());

            camera_distort_->update_mask(input_mask, output_mask);
            seg_by_contours_->PostProcess(output_mask, vec_labels, item.seg_contours);

            for (auto seg_iter = item.seg_contours.begin(); seg_iter != item.seg_contours.end();) {
                ...
                if (seg_volumes_.count(label) > 0) {
                    ...
                    seg_iter = item.seg_contours.erase(seg_iter);
                }
            }
        }
    }
