...
void Graphics_v2::on_cv_image(const std::shared_ptr<msgs::cv_frame> &frame) {
    ...
    for (auto &[key, points] : frame->frame_info->roi_points) { graphics_->draw_polygon_fill(points, {116, 0, 0, 255 * 0.2}); }
    ...
    frame->frame_info->tgt_frame = graphics_->get_image().clone();
