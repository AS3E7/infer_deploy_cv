...
void RoiCrop_v2::on_setup() {
    if (regions_with_label_.empty()) {
        int region_id_ = 0;
        for (auto &region : regions_) { regions_with_label_[std::to_string(region_id_++)] = region; }
    }

    spdlog::info("############################ CROP ROI ############################");
    for (auto &[key, points] : regions_with_label_) {
        spdlog::info("[");
        for (auto &point : points) { spdlog::info("    [{}, {}]", point[0], point[1]); }
        spdlog::info("]");
    }
    spdlog::info("#################################################################");
}
