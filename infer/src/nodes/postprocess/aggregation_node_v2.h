...
public:
    explicit Aggregation_v2(std::string name) : node_any_basic(std::move(name)) {
        bind_simple_property("threshold", threshold_, "IOU阈值");
        bind_simple_property("merge_same", merge_same_, "合并同目标");
        bind_simple_property("only_most_match", only_most_match_, "只保留置信度最高目标");
...
private:
    ...
    bool merge_same_{false};
    bool only_most_match_{false};
