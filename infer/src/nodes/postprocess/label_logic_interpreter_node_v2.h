...
public:
    explicit LabelLogicInterpreter_v2(std::string name) : node_any_basic(std::move(name)) {
        bind_simple_property("expression", str_expression_, "逻辑表达式");
        bind_simple_property("map_label", map_label_, "是否映射标签");
        bind_simple_property("expression_true", expression_true_, "表达式成立输出");
        bind_simple_property("expression_false", expression_false_, "表达式不成立输出");
...
private:
    bool map_label_{false};
    std::string str_expression_;
    std::string expression_true_;
    std::string expression_false_;
