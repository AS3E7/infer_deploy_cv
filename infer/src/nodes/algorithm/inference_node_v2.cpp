        ...
        thread_handle_.emplace_back(std::thread([this, i]() {
                ...
                try {
                    ...
                    if (mod_label_objs_.count("all")) {
                        for (auto &item : vec_labels) {
                            ...
                            map_class_color_[index] = {0, 0, 255, 255 * 0.8};
                            index++;
                        }
                    } else {
                        for (auto &item : vec_labels) {
                            if (mod_label_objs_.count(item) > 0 && mod_label_objs_[item]["checked"].get<bool>()) {
                                ...
                                auto color = mod_label_objs_[item]["color"].get<std::vector<float>>();
                                map_class_color_[index] = {color[0], color[1], color[2], 255 * 0.8};
                            }
                            index++;
                        }
                    }
            ...
            algo_impl->register_infer_callback(
                [this, &infer_mtx, &infer_frames](const int64_t frame_idx, const AlgoType type,
                    ...
                    // bbox 越界处理
                    for (auto &item : const_cast<std::vector<algo::AlgoOutput> &>(vec_output)) {
                        item.box.x = std::max(0.0f, item.box.x);
                        item.box.y = std::max(0.0f, item.box.y);
                        item.box.width = std::min(frame->frame_info->width() - item.box.x, item.box.width);
                        item.box.height = std::min(frame->frame_info->height() - item.box.y, item.box.height);
                    }

                    frame->frame_info->ext_info.emplace_back(parser_output(type, frame->frame_info, vec_output));
                    ...
                    if (future.valid()) {
                        if (frame->task_type == TaskType::kImage || frame->task_type == TaskType::kAsyncImage
                            ...
                            frame->frame_info->ext_info.emplace_back(
                                parser_output(AlgoType::kUndefined, frame->frame_info, {}));
                            output_result_(frame);
                            continue;
                        }
                    }
                        ...
                        if (frame->frame_info->ext_info.empty()) {
                            ...
                            for (auto &[_, output] : outputs) {
                                for (auto &item : output) {
                                    ...
                                    item.box.width = std::min(frame->frame_info->width() - item.box.x, item.box.width);
                                    item.box.height =
                                        std::min(frame->frame_info->height() - item.box.y, item.box.height);
                                }
                            }
                            frame->frame_info->ext_info.emplace_back(
...
void Inference_v2::on_cv_frame(const std::shared_ptr<msgs::cv_frame> &frame) {
    ...
    if (cache_frames_.size_approx() > 20) {
        ...
        std::shared_ptr<msgs::cv_frame> tmp_frame;
        cache_frames_.try_dequeue(tmp_frame);
    }

    cache_frames_.enqueue(std::make_shared<msgs::cv_frame>(frame));
}

nodes::FrameExtInfo Inference_v2::parser_output(const AlgoType type,
    ...
    for (auto &item : map_class_color_) {
        ext_info.map_class_color[item.first] = {item.second[0], item.second[1], item.second[2], item.second[3]};
    }
    ...
    for (const auto &item : vec_output) {
        ...
        // 已勾选类别
        if (type == AlgoType::kSegmentation) {
            SegInfo seg_info;
            seg_info.seg_width = item.seg_width;
            seg_info.seg_height = item.seg_height;

            std::vector<uint8_t> seg_map(item.seg_width * item.seg_height, 0);
            for (int i = 0; i < item.seg_width * item.seg_height; i++) {
                if (map_class_label_.count(item.seg_map[i]) > 0) { seg_map[i] = item.seg_map[i]; }
            }
            seg_info.seg_map = std::move(seg_map);
            ext_info.map_seg_info[index] = std::move(seg_info);
...
nodes::FrameExtInfo Inference_v2::parser_output(const AlgoType type, const FrameExtInfo &back_ext_info,
    ...
    for (const auto &[prev_id, output] : outputs) {
        for (const auto &item : output) {
            ...
            if (type == AlgoType::kOCR_REC || type == AlgoType::kOCR) {
                for (auto ocr_info : item.vec_ocr_info) { ext_info.map_ocr_info[prev_id] = ocr_info; }
                ...
                if (map_class_label_.count(item.class_id) > 0) {
                    ...
                    if (type == AlgoType::kDetection) {
                        ...
                        seg_info.seg_width = item.seg_width;
                        seg_info.seg_height = item.seg_height;
                        seg_info.seg_map = std::move(item.seg_map);
                        ext_info.map_seg_info[index] = std::move(seg_info);
                    }
                }
