...
#include "modules/wrapper/cn_jpeg_codec.h"
#include "modules/wrapper/nvidia_wrapper.hpp"
#include "modules/wrapper/rkmedia_jpeg_codec.h"
...
public:
        ...
        register_input_message_handler_<msgs::cv_frame>([=](const std::shared_ptr<msgs::cv_frame> &frame) {
            ...
            if (masking_) {
                ...
                if (frame->frame_info->ext_info.front().infer_target_info.empty()) {
                    ...
                    if (!jpeg_encoder_) {
                        jpeg_encoder_ = std::make_unique<codec::RkMediaJpegCodec>();
                        if (!jpeg_encoder_->init_codecer(frame->frame_info->width() / scale_,
                                                         frame->frame_info->height() / scale_, 50)) {
                            jpeg_encoder_.reset();
                            return;
                        }
                    }

                    jpeg_encoder_->codec_image(frame->frame_info->src_frame->data->data, jpeg_data_);
                    ...
                    image_wrapper::image_jpeg_enc(frame->frame_info->src_frame->data, jpeg_data_, jpeg_data_size, 85);
#endif
                }
            }
            ...
            if (!front_ext_info.tracked_box.empty()) {
                for (const auto &[track_id, item] : front_ext_info.tracked_box) {
                    ...
                    box_obj["label"] =
                        front_ext_info.map_class_label.at(item.class_id) + "-" + std::to_string(track_id);
                    box_obj["color"] = front_ext_info.map_class_color.at(item.class_id).to_array();
                    box_array.push_back(box_obj);

                    if (front_ext_info.algo_type == AlgoType::kAction) {
                        auto box_obj = nlohmann::json::object();
                        box_obj["left_top_x"] = item.box.x / scale_;
                        ...
                        if (front_ext_info.sum_action_scores.count(track_id) > 0) {
                            box_obj["prob"] = front_ext_info.sum_action_scores.at(track_id).begin()->second.size();
                        } else {
                            box_obj["prob"] = 0;
                        }
                        box_obj["label"] = "total";
                ...
                for (const auto &[target_id, item] : front_ext_info.map_target_box) {
                    auto box_obj = nlohmann::json::object();
                    box_obj["left_top_x"] = item.box.x / scale_;
                    box_obj["left_top_y"] = item.box.y / scale_;
                    box_obj["right_bottom_x"] = (item.box.x + item.box.width) / scale_;
                    ...
                    box_obj["prob"] = item.prob;
                    box_obj["label"] =
                        front_ext_info.map_class_label.at(item.class_id) + "-" + std::to_string(target_id);
                    box_obj["color"] = front_ext_info.map_class_color.at(item.class_id).to_array();
                    box_array.push_back(box_obj);
                }
            }
            ...
            /*************************** 越界计数可视化处理 **************************/
            int len = front_ext_info.border_points.size();
            for (int i = 0; i < len; i++) {
                auto &points = front_ext_info.border_points[i];
                auto points_array = nlohmann::json::array();
                int len = points.size();
                for (int j = 0; j < len - 1; j++) {
                    auto point_obj = nlohmann::json::object();
                    point_obj["start_x"] = points[j].x;
                    point_obj["start_y"] = points[j].y;
                    point_obj["end_x"] = points[j + 1].x;
                    point_obj["end_y"] = points[j + 1].y;
                ...
                int index = 0;
                for (auto &[key, values] : front_ext_info.cross_count[i]) {
            ...
            /******************************** 分割 可视化 ****************************/
            for (const auto &[_, item] : back_ext_info.map_seg_info) {
            ...
            jpeg_encoder_->codec_image(tgt_frame.data, jpeg_data_);
            auto jpeg_data_size = jpeg_data_.size();
            ...
            if (!jpeg_encoder_) {
                jpeg_encoder_ = std::make_unique<codec::CNJpegCodec>();
                if (!jpeg_encoder_->init_codecer(codec::JpegCodecParam{.type = codec::CodecType::kEncoder,
                                                                       .width = frame->frame_info->width() / scale_,
                                                                       .height = frame->frame_info->height() / scale_,
                                                                       .format = codec::PixelFormat::KNV12,
                                                                       .quality = 85})) {
                    jpeg_encoder_.reset();
                    return;
                }
            }
            jpeg_encoder_->codec_image(tgt_frame, [&buffer, &box_string](const uint8_t *data, const uint32_t dlen) {
                buffer = std::vector<uchar>(4 + dlen + box_string.size());
                memcpy(buffer.data(), &dlen, sizeof(dlen));
                memcpy(buffer.data() + sizeof(dlen), data, dlen);
                memcpy(buffer.data() + sizeof(dlen) + dlen, box_string.data(), box_string.size());
            });
#elif defined(WITH_TX5368)
            if (!jpeg_encoder_) {
                jpeg_encoder_ = std::make_unique<codec::TsingJpegEncode>();
...
#else
            int jpeg_data_size = 0;
            if (!masking_) { image_wrapper::image_jpeg_enc(tgt_frame, jpeg_data_, jpeg_data_size, 85); }
            buffer = std::vector<uchar>(4 + jpeg_data_size + box_string.size());
            memcpy(buffer.data(), &jpeg_data_size, sizeof(jpeg_data_size));
            memcpy(buffer.data() + sizeof(jpeg_data_size), jpeg_data_.data(), jpeg_data_size);
            memcpy(buffer.data() + sizeof(jpeg_data_size) + jpeg_data_size, box_string.data(), box_string.size());
    ...
    void on_setup() override {
        jpeg_data_ = std::vector<uint8_t>(4096 * 2160);
        const char *env_value = std::getenv("JPEG_PREVIEW_ADDRESS");
        if (env_value != nullptr) { address_ = env_value; }
    }
...
#if defined(WITH_RV1126)
    std::unique_ptr<codec::RkMediaJpegCodec> jpeg_encoder_;
#elif defined(WITH_MLU220)
    std::unique_ptr<codec::CNJpegCodec> jpeg_encoder_;
#elif defined(WITH_TX5368)
