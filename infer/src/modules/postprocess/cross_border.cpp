...
public:
    ...
    bool init_border(const Position &position, const std::vector<Point2i> &border, const int margin) {
        position_ = position;
        border_ = border;
        margin_ = margin;
    ...
    std::map<int, int> update_line(const std::map<int, Rect2f> &rects) {
        ...
        for (const auto &[track_id, rect] : rects) {
            ...
            int cur_x = rect.x + rect.width / 2;
            int cur_y = rect.y + rect.height / 2;
            if (position_ == Position::kTop) {
                cur_x = rect.x + rect.width / 2;
                cur_y = rect.y;
            } else if (position_ == Position::kRight) {
                cur_x = rect.x + rect.width;
                cur_y = rect.y + rect.height / 2;
            } else if (position_ == Position::kBottom) {
                cur_x = rect.x + rect.width / 2;
                cur_y = rect.y + rect.height;
            } else if (position_ == Position::kLeft) {
                cur_x = rect.x;
                cur_y = rect.y + rect.height / 2;
            }

            float distance = calculate_distance(Point2i{cur_x, cur_y}, border_);

            if (track_id_status.size() > 150) { track_id_status.erase(track_id_status.begin()); }

            spdlog::debug("================ {}", distance);
            if (distance >= 0) {
                track_id_status.emplace_back(direction(Point2i{cur_x, cur_y}, border_));
...
private:
    Position position_{Position::kCenter};
    int margin_{0};
...
bool CrossBorder::init_border(const std::vector<Point2i> &border, const int margin) {
    ...
    return cross_border_[0].init_border(Position::kCenter, border, margin);
}

bool CrossBorder::init_border(const Position &position, const std::vector<std::vector<Point2i>> &borders,
                              const int margin) {
    for (const auto &item : borders) {
        CrossBorderPrivate crosser;
        if (crosser.init_border(position, item, margin)) {
