		...
		const int n_classes)
{
	...
	this->m_new_track_thresh = det_thresh;
	this->m_unconfirmed_match_thresh = unconfirmed_thresh;
	this->m_high_det_thresh = conf_thresh;
	this->m_low_det_thresh = low_conf_thresh;
	this->m_high_match_thresh = match_thresh;
	this->m_low_match_thresh = low_match_thresh;
	this->m_max_time_lost = max_frame_lost;
}
...
unordered_map<int, vector<Track>> BYTETracker::updateMCMOT(const vector<TargetObject>& objects)
{
	// frame id updating
	this->m_frame_id++;
	...
	unordered_map<int, vector<int>> boxid_dict;
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); ++i)
		{
			const TargetObject& obj = objects[i];
			...
			boxid_dict[cls_id].push_back(obj.box_id);
		}
	}  // non-empty objects assumption

	// ---------- Processing each TargetObject classes
	...
	for (int cls_id = 0; cls_id < this->m_N_CLASSES; ++cls_id)
	{
		...
		// class scores
		const vector<int>& cls_boxid = boxid_dict[cls_id];
		...
		for (int i = 0; i < cls_bboxes.size(); ++i)
		{
			...
			Track track(Track::tlbrTotlwh(tlbr_), score, cls_id, cls_boxid[i]);
			if (score > this->m_high_det_thresh)  // high confidence dets
			{
				cls_dets.push_back(track);
			}
			else if(score > m_low_det_thresh) // low confidence dets
			{
				cls_dets_low.push_back(track);
			}
		}
		// printf("zzy debug cls_dets size %d\n",cls_dets.size());
		...
		// printf("zzy debug first association activated_tracks size %d\n",activated_tracks_dict[cls_id].size());
		...
		// printf("zzy debug second association activated_tracks size %d\n",activated_tracks_dict[cls_id].size());
...
#if 0
vector<Track> BYTETracker::update(const vector<TargetObject>& objects)
{
	// frame id updating
	this->m_frame_id++;
