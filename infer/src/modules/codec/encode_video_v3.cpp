...
bool Encoder_v3::encode_frame(const std::shared_ptr<AVFrame> &frame) {
    if (impl_) { return impl_->encode_frame(frame); }
    return true;
}

bool Encoder_v3::encode_frame(const uint8_t *data, int width, int height) {
    if (impl_) { return impl_->encode_frame(data, width, height); }
    return true;
}
