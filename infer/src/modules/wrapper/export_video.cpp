...
bool ExportVideo::init_video(const std::string &output_url, const int frame_rate, const int width, const int height) {
    ...
    try {
        ...
        if (!encoder_->open_encoder(
                av_wrapper::EncodeOptions{.width = width, .height = height, .framerate = frame_rate}, DEVICE_TYPE)) {
