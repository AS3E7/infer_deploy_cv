...
void DrawImage::draw_frame(const std::shared_ptr<nodes::FrameInfo> &frame_info) {
    auto &front_ext_info = frame_info->ext_info.front();
    ...
    if (!blit_image_.empty()) {
        ...
        cur_blit_image = graphics_->get_image().clone();
    }

    graphics_->set_image(image_wrapper::image_to_mat(frame_info->src_frame->data));

    // 越线计数可视化
    for (const auto &points : front_ext_info.border_points) {
    ...
    // 目标框可视化
    for (const auto &[_, item] : front_ext_info.tracked_box) {
        auto color = front_ext_info.map_class_color.at(item.class_id);
        graphics_->draw_rect(item.box, item.prob, color, 2);
        graphics_->draw_text_fill(
            Point2i{(int)item.box.x - 3, (int)item.box.y},
            front_ext_info.map_class_label.at(item.class_id) + " " + std::to_string(item.prob).substr(0, 4), color);
    }
    ...
    if (front_ext_info.tracked_box.empty()) {
        for (const auto &[_, item] : back_ext_info.map_target_box) {
            ...
            graphics_->draw_text_fill(
                Point2i{(int)item.box.x - 3, (int)item.box.y},
                back_ext_info.map_class_label.at(item.class_id) + " " + std::to_string(item.prob).substr(0, 4), color);
        }
    }
    ...
    for (const auto &[_, points] : back_ext_info.map_key_points) {
        if (points.size() == 17) {
            ...
            for (const auto &bone : skeleton_5) {
                graphics_->draw_line(Point2i{(int)points[bone[0]].x, (int)points[bone[0]].y},
                                     Point2i{(int)points[bone[1]].x, (int)points[bone[1]].y}, Scalar({0, 255, 0, 255}),
                                     2);
            }
        }
    ...
    for (const auto &[_, item] : back_ext_info.map_ocr_info) {
        ...
        graphics_->draw_text_fill(Point2i{(int)item.points[0].x, (int)item.points[0].y}, text, Scalar{255, 0, 0, 255});
    }
