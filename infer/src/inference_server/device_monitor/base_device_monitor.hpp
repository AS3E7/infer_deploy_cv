...
public:
    ...
    virtual std::string get_device_serial_number() = 0;

    virtual std::string get_chip_name() = 0;
...
#if !defined(WITH_BM1684) && !defined(WITH_NVIDIA) && !defined(WITH_MLU220) && !defined(WITH_MLU270)                   \
    && !defined(WITH_INTEL) && !defined(WITH_JETSON) && !defined(WITH_RV1126) && !defined(WITH_MLU370)                 \
    && !defined(WITH_TX5368)
std::unique_ptr<BaseDeviceMonitor> make_device_monitor() { return std::make_unique<BaseDeviceMonitor>(); }
#endif
