...
from utils.general import (coco80_to_coco91_class, check_dataset, check_file,
                           ...
                           non_max_suppression_one_scale, scale_coords,
...
def test(
    ...
    # Show FLOPs
    #    if not training:
    #        flops_count, params_count = get_model_complexity_info(model,
    #                                                              (3,imgsz,imgsz),
    #                                                              as_strings=False,
    #                                                              print_per_layer_stat=True)
    #        print('Input resolution: {} x {}, FLOPs: {}'.format(imgsz, imgsz, flops_count*2/1e9))
        ...
        with torch.no_grad():
            ...
            if training:  # if model has loss hyperparameters
                loss += compute_loss([x.float() for x in train_out], targets,
                                     model, model.hyp)[1][:3]  # GIoU, obj, cls
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    if task != 'val':
        # flops_count, params_count = get_model_complexity_info(model,(3,imgsz,imgsz))
        # print('Input resolution: {} x {}, MACs: {}'.format(imgsz, imgsz, flops_count/1e9))
        # print(flops_count, params_count)
        # print('flopsflospflosp',flops_count)
        # print('paramsparamsparams',params_count)
        model_info = {}
        # model_info['params'] = float(params_count.split()[0])
        # model_info['flops'] = float(flops_count.split()[0])
        model_info['mAP'] = map50

        # json_fp = open(os.path.join('/gddi_output','result.json'), "w",encoding='utf-8')
        # json_str = json.dumps(model_info, indent=4)
        # json_fp.write(json_str)
        # json_fp.close()
        # print('test results', map50, map)
    # print('\n\n\n\n\n\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data',
                        type=str,
                        default='data/global-wheat.yaml',
                        help='*.data path')
    ...
    if opt.task in ['val', 'test']:  # run normally
        test(opt.data, opt.weights, opt.batch_size, opt.img_size,
             ...
             opt.augment, opt.verbose, opt.task)
