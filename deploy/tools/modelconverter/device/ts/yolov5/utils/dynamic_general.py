...
def bn_calibration_init(m):
    ...
    if hasattr(m, 'track_running_stats'):
        ...
        if m.running_mean is None:
            m.running_mean = nn.Parameter(torch.zeros(m.num_features).cuda())
        if m.running_var is None:
            m.running_var = nn.Parameter(torch.ones(m.num_features).cuda())
        if m.num_batches_tracked is None:
            m.num_batches_tracked = nn.Parameter(torch.zeros(1).cuda())
...
def compute_loss(p, targets, model, hyp):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                t[range(n), tcls[i]] = cp
                if hyp['reweight']:
                    BCEcls.weight=torch.full_like(t, 1.0, device=device)
                    labels = t.argmax(dim=1)
                    for reweight_class in hyp['reweight_class']:
                        if (labels == reweight_class).any():
                            BCEcls.weight[labels == reweight_class] = 2.0
                lcls += BCEcls((ps[:, 5:] - margin * t) / temp, t)  # BCE
    ...
    loss = lbox + lobj + lcls
    return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach(), [i.detach() for i in p], indices
