'''
opyright (C) 2019 Sovrasov V. - All Rights Reserved
...
from models.dynamic_common import DYConv2d


def dynamic_conv_flops_counter_hook(conv_module, input, output):
    input = input[0]
    output = output[0]
...
def print_model_with_flops(model, total_flops, total_params, units='GMac',
    ...
    def flops_repr(self):
        ...
        return ', '.join([params_to_string(accumulated_params_num,
                          ...
                          '{:.3%} Params'.format(accumulated_params_num / (total_params + 1)),
                          flops_to_string(accumulated_flops_cost,
                                          units=units, precision=precision),
                          '{:.3%} MACs'.format(accumulated_flops_cost / (total_flops + 1)),
...
MODULES_MAPPING = {
    ...
    # Dynamic OPs
    DYConv2d: dynamic_conv_flops_counter_hook
}
