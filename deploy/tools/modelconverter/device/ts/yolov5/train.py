...
from utils.general import (torch_distributed_zero_first,
                           labels_to_class_weights, plot_labels, check_anchors,
                           labels_to_image_weights, compute_loss, plot_images,
                           fitness, strip_optimizer, plot_results,
                           get_latest_run, check_dataset, check_file,
                           check_git_status, check_img_size, increment_dir,
                           print_mutation, plot_evolution, set_logging)
...
if __name__ == '__main__':
    ...
    if opt.resume:  # resume an interrupted run
        ...
        log_dir = Path(opt.logdir) / opt.name  # runs/exp1
    ...
    # Evolve hyperparameters (optional)
        ...
        yaml_file = Path(
            'runs/evolve/hyp_evolved.yaml')  # save best result here
        if opt.bucket:
