...
//
int ClassifyDecodeOutputNCHW(std::vector<gddeploy::BufSurfWrapperPtr> out_data, 
...
{   
    ...
    for (size_t b = 0; b < out_data.size(); b++) {
        ...
        result.result_type.emplace_back(GDD_RESULT_TYPE_CLASSIFY);
        result.classify_result.batch_size++;
...
int ClassifyDecodeOutputNHWC(std::vector<gddeploy::BufSurfWrapperPtr> out_data, 
                                    gddeploy::InferResult &result,  
                                    gddeploy::any param,  
                                    std::vector<gddeploy::FrameInfo> frame_info, 
                                    gddeploy::ModelPtr model_ptr)
