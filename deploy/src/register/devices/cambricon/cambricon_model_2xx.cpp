...
public:
    Cambricon2xxModelPrivate(const std::string& model_path,const std::string& key);
    Cambricon2xxModelPrivate(void* mem_ptr,  size_t mem_size, const std::string& key);
    ~Cambricon2xxModelPrivate();
    ...
    std::shared_ptr<cnrtFunction_t> GetModel() { return std::shared_ptr<cnrtFunction_t>(function_); }
...
static DataType convertCambricon2xxDataType2MyDataType(cnrtDataType_t in){
    DataType out;
...
Cambricon2xxModelPrivate::Cambricon2xxModelPrivate(const std::string& model_path, const std::string& key)
{
    cnrtRet_t error_code = cnrtLoadModel(&model_, model_path.c_str());

    function_ = std::shared_ptr<cnrtFunction_t>(new cnrtFunction_t(), [](cnrtFunction_t *function) {
        cnrtRet_t error_code = cnrtDestroyFunction(*function);
    });
    error_code = cnrtCreateFunction(function_.get());
    // printf(error_code, "[EasyDK InferServer] [Model] Create function failed.", false);
    error_code = cnrtExtractFunction(function_.get(), model_, func_name_.c_str());
...
Cambricon2xxModelPrivate::Cambricon2xxModelPrivate(void* mem_ptr,  size_t mem_size, const std::string& key)
{
...
Cambricon2xxModelPrivate::~Cambricon2xxModelPrivate()
{
    auto error_code = cnrtUnloadModel(model_);
}

int Cambricon2xxModelPrivate::SetInputOuputInfo()
{
    ...
    dim_num = 0;
    error_code = cnrtGetOutputDataType(&output_dtypes_, &output_num, *function_);
...
int Cambricon2xxModel::Init(const std::string& model_path, const std::string& param)
{
    cambricon2_model_priv_ = std::make_shared<Cambricon2xxModelPrivate>(model_path, param);

    auto input_dims = cambricon2_model_priv_->GetInputNodeDims();
    for (auto dims : input_dims){

        inputs_shape_.push_back(Shape(dims));
    }
    
    auto input_data_type = cambricon2_model_priv_->GetInputDataType();
    for (auto dti : input_data_type){
        DataLayout dl;
        dl.dtype = convertCambricon2xxDataType2MyDataType(dti);
        dl.order = DimOrder::NHWC;
        intput_data_layout_.push_back(dl);
    }

    auto output_dims = cambricon2_model_priv_->GetOutputNodeDims();
    for (auto dims : output_dims)
        outputs_shape_.push_back(Shape(dims));

    auto output_data_type = cambricon2_model_priv_->GetOutputDataType();
...
int Cambricon2xxModel::Init(void* mem_ptr, size_t mem_size, const std::string& param)
{
    cambricon2_model_priv_ = std::make_shared<Cambricon2xxModelPrivate>(mem_ptr, mem_size, param);
 ...
 */
const Shape& Cambricon2xxModel::InputShape(int index) const noexcept
{
 ...
 */
const Shape& Cambricon2xxModel::OutputShape(int index) const noexcept
{
 ...
 */
int Cambricon2xxModel::FixedOutputShape() noexcept
{
 ...
 */
const DataLayout& Cambricon2xxModel::InputLayout(int index) const noexcept
{
 ...
 */
const DataLayout& Cambricon2xxModel::OutputLayout(int index) const noexcept
{
 ...
 */
uint32_t Cambricon2xxModel::InputNum() const noexcept
{
 ...
 */
uint32_t Cambricon2xxModel::OutputNum() const noexcept
{
 ...
 */
uint32_t Cambricon2xxModel::BatchSize() const noexcept
{
 ...
 */
std::string Cambricon2xxModel::GetKey() const noexcept
{
    return cambricon2_model_priv_->GetMD5();
}


any Cambricon2xxModel::GetModel()
{
    return cambricon2_model_priv_->GetModel();
}


// REGISTER_MODEL_CREATOR("any", "any", Cambricon2xxModelCreator) 
