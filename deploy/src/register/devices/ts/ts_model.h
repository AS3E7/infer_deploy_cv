...
class TsModel : public Model {
public:
    TsModel(const any& value):Model(){}
    ~TsModel() = default;
...
private:
    ...
    std::shared_ptr<TsModelPrivate> ts_model_priv_; 
...
class TsModelCreator : public ModelCreator{
public:
    TsModelCreator():ModelCreator("TsModelCreator"){
    }

    std::string GetName() const override { return model_creator_name_; }

    std::shared_ptr<Model> Create(const any& value) override {
        return std::shared_ptr<TsModel>(new TsModel(value));
    }

private:
    std::string model_creator_name_ = "TsModelCreator";

};  // class TsModelCreator
}