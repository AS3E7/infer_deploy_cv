...
public:
    AscendModel(const any& value):Model(){}
    ~AscendModel() = default;
...
private:
    ...
    std::shared_ptr<AscendModelPrivate> ascend_model_priv_; 
...
class AscendModelCreator : public ModelCreator{
public:
    AscendModelCreator() : ModelCreator("AscendModelCreator"){
    }

    std::string GetName() const override { return model_creator_name_; }

    std::shared_ptr<Model> Create(const any& value) override {
        return std::shared_ptr<AscendModel>(new AscendModel(value));
    }

private:
    std::string model_creator_name_ = "AscendModelCreator";

};  // class AscendModelCreator
}
