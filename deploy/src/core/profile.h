#ifndef GDDEPLOY_CORE_PROFILE_H_
#define GDDEPLOY_CORE_PROFILE_H_
...
class Profiler : public Clock {
 public:
  ...
  void RequestEnd(uint32_t unit_cnt) noexcept {
    if (processing_cnt_ == 0u)
      GDDEPLOY_ERROR("processing_cnt_ is 0");
    std::lock_guard<std::mutex> lk(mutex_);
...
class TagSetProfiler {
 public:
  ...
  float RequestPerSecond(const std::string& tag) noexcept {
    ...
    GDDEPLOY_WARN("Tag [{}] not exist", tag);
    return 0;
  }
...
#endif  // GDDEPLOY_CORE_PROFILE_H_
