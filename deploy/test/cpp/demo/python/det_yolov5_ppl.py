...
if __name__ == '__main__':
    ...
    parser.add_argument('--ppl', default='', required=True)
    ...
    # with open('./models/gddi_model_quantized.json', 'r') as f:
    ...
    status = runtime_builder.InitFromFile(args.ppl, [cuda_engine])
    ...
    # status = runtime_builder.Serialize('./models/gddi_model_quantized', "pmx")
    # if status != pplcommon.RC_SUCCESS:
    ...
    status = tensor.ConvertFromHost(img)
    if status != pplcommon.RC_SUCCESS:
    ...
    tensor0 = np.array(runtime.GetOutputTensor(0).ConvertToHost(), copy=False)
    tensor1 = np.array(runtime.GetOutputTensor(1).ConvertToHost(), copy=False)
    tensor2 = np.array(runtime.GetOutputTensor(2).ConvertToHost(), copy=False)
